			...
			<div class="city_content">
				...
				<div class="tourist">
					<div class="content">
						<p>${area.contents}</p>
						<div onclick="initMap()">
							<a href="javascript:void(0)" onclick="openPanorama()" class="button btn1 panorama_btn">
								파노라마 뷰 보러가기 <i class="fas fa-arrow-right"></i>
							</a>
						</div>
					</div>
					<div class="AIcontent">
						...
						<a href="javascript:void(0)" onclick="openMapUrl('recommendPlaces.do')" class="button btn1 map_btn"> 경로추천 보러가기 </a>
					</div>
				</div>
			</div>
			<div class="comment">
				...
				<form name="commentform" action="${ctx}/commentAdd.do" method="post">
					...
					<input type="hidden" name="no" value="${area.no}">
					<input type="hidden" name="areaId" value="${area.id}">
					<input type="text" name="comment" id="" placeholder="댓글 입력...">
					<input type="submit" class="button btn" value="등록" <c:if test="${loginId eq null}">disabled="disabled"</c:if>>
				</form>
				<ul class="comment">
