...
<!DOCTYPE html>
<html style="background-image: linear-gradient(#00000033, #00000033), url('${ctx}/img/background.jpg')">
<head>
<meta charset="UTF-8">
<title>EYEVEL</title>
		...
		<div class="innerBox">
			...
			<ul class="gnb">
				<li class="mobildeNone" <c:if test="${area eq 'all'}">class="on mobildeNone"</c:if>>
					<a href="${ctx}/areaEarth.do?area=all">전체</a>
				</li>
				<li <c:if test="${area eq 'asia'}">class="on"</c:if>>
					<a href="${ctx}/areaContinent.do?area=asia">아시아</a>
				</li>
				<li <c:if test="${area eq 'africa'}">class="on"</c:if>>
					<a href="${ctx}/areaContinent.do?area=africa">아프리카</a>
				</li>
				<li <c:if test="${area eq 'northAmerica'}">class="on"</c:if>>
					<a href="${ctx}/areaContinent.do?area=northAmerica">북아메리카</a>
				</li>
				<li <c:if test="${area eq 'southAmerica'}">class="on"</c:if>>
					<a href="${ctx}/areaContinent.do?area=southAmerica">남아메리카</a>
				</li>
				<li <c:if test="${area eq 'europe'}">class="on"</c:if>>
					<a href="${ctx}/areaContinent.do?area=europe">유럽</a>
				</li>
				<li <c:if test="${area eq 'oceania'}">class="on"</c:if>>
					<a href="${ctx}/areaContinent.do?area=oceania">오세아니아</a>
				</li>
				<li <c:if test="${area eq 'antarctica'}">class="on"</c:if>>
					<a href="${ctx}/areaContinent.do?area=antarctica">남극</a>
				</li>
			</ul>
		</div>
	</header>
