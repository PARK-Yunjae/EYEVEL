...
<section class="boardSection">
	<div class="innerBox">
		...
		<div class="search">
			<input type="text" id="searchText" placeholder="검색" <c:if test="${!empty searchText}">
					value="${searchText}"
				</c:if>>
			<a onclick="boardSearchText()">
		...
		<ul class="category">
			<li class=
				...
				<c:if test="${empty category || category eq 'all'}">"all on"</c:if>>전체</li>
			<li class=
				<c:if test="${category ne 'notice'}">"notice"</c:if> 
				<c:if test="${category eq 'notice'}">"notice on"</c:if>>공지사항</li>
			<li class=
				<c:if test="${category ne 'complain'}">"complain"</c:if> 
				<c:if test="${category eq 'complain'}">"complain on"</c:if>>건의사항</li>
		</ul>
		<table>
			<tr>
				<th>No</th>
			...
			<c:forEach var="board" items="${list}">
				<tr>
					...
					<td>
						<a href="${ctx}/boardInfo.do?no=${board.no}">${board.title}</a>
					</td>
		...
		<ul class="paging">
			<c:if test="${startPage>3}">
				<li class="start" onclick="location.href='${ctx}/boardList.do?page=${startPage-3}&start=${startPage-3}&searchText=${searchText}&category=${category}'">
			...
			<c:if test="${totalPage>endPage}">
				<li class="end" onclick="location.href='${ctx}/boardList.do?page=${startPage+3}&start=${startPage+3}&searchText=${searchText}&category=${category}'">
