...
<section>
	<div class="innerBox">
		...
		<form
			action=<c:if test="${board.title eq null}">"${ctx}/boardAdd.do"</c:if>
			<c:if test="${board.title ne null}">"${ctx}/boardUpdate.do"</c:if>
			method="post" class="add_board">
			...
			<div class="input_info">
				...
				<div class="input_date">
					<label for="date">작성일</label><br> <input type="text"
			...
			<c:if test="${loginId eq 'admin'}">
				<div class="notice_check">
					<input type="checkbox" id="notice" name="notice"> <label
						for="notice" class="notice_label">공지사항으로 등록하기</label>
				</div>
			</c:if>
			<c:if test="${board.title eq null}">
				<input type="button" class="button btn" value="등록하기"
					onclick="boardAddCheck(form)">
			</c:if>
			<c:if test="${board.title ne null}">
				<input type="button" class="button btn" value="수정하기"
					onclick="boardAddCheck(form)">
			</c:if>
		</form>
	</div>
