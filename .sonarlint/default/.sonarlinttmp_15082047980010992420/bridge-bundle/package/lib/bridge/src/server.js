...
function start(port = 0, host = '127.0.0.1', timeout = SHUTDOWN_TIMEOUT) {
    ...
    return new Promise(resolve => {
        (0, helpers_1.debug)('Starting the bridge server');
        worker = new worker_threads_1.Worker(path_1.default.resolve(__dirname, 'worker.js'), {
            workerData: { context: (0, helpers_1.getContext)() },
            env: worker_threads_1.SHARE_ENV,
        });
        worker.on('online', () => {
            (0, helpers_1.debug)('The worker thread is running');
        });
        worker.on('exit', code => {
            (0, helpers_1.debug)(`The worker thread exited with code ${code}`);
        });
        worker.on('error', err => {
            (0, helpers_1.debug)(`The worker thread failed: ${err}`);
            ...
            (0, helpers_1.debug)('Shutting down the bridge server due to failure');
            shutdown();
        });
        ...
        app.post('/close', (_, response) => {
            (0, helpers_1.debug)('Shutting down the bridge server');
            response.end(() => {
                shutdown();
            });
        });
        server.on('close', () => {
            (0, helpers_1.debug)('The bridge server shut down');
            orphanTimeout.stop();
        });
        server.on('error', err => {
            (0, helpers_1.debug)(`The bridge server failed: ${err}`);
        });
        server.on('listening', () => {
            var _a;
            /**
             ...
             */
            (0, helpers_1.debug)(`The bridge server is listening on port ${(_a = server.address()) === null || _a === void 0 ? void 0 : _a.port}`);
            resolve(server);
        });
         ...
         */
        function shutdown() {
            worker.terminate().catch(reason => (0, helpers_1.debug)(`Failed to terminate the worker thread: ${reason}`));
            server.close();
        }
    });
}
